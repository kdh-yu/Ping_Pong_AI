...
EXPLORE = 500000
OBSERVE = 50000
#store our expiriences, the size of it
...
def createGraph():
    ...
    W_conv2 = tf.Variable(tf.zeros([4, 4, 32, 364]))
    b_conv2 = tf.Variable(tf.zeros([32]))

    W_conv3 = tf.Variable(tf.zeros([3, 3, 64, 364]))
    ...
    #Computes rectified linear unit activation function on a 2D convolution given 4D input and filter tensor
...
#deep Q learning. feed in pixel data to graph session
def trainGraph(inp, out, sess):
    ...
    #action
    action = tf.reduce_sum(tf.mul(out, argmax), refuction_indices = 1)
    #cost function we will refuce through backpropagation
    cost = tf.reduce_mean(tf.square(action - gt))
    #optimazation function to reduce and minimize our cost function
    train_step = tf.train.AdamOptimizer(1e-6).minimize(cost)

    #initialize our game
    game = PongGame.PongGame()

    #create q queue for experince replay to store policies
    D = deque()

    #initial frame
    frame = game.getPresentFame()
    ...
    while(1):
        ...
        #if we run out of replay memory, make a room
        ...
        if t > OBSERVE:
            ...
            train_step.run(feed_dict = {
                gt : gt_batch,
                argmax_t : argmax_batch,
                inp : inp_batch
            })

        #update our input tensor for the next frame
        inp_t = inp_t1
        t = t+1

        #print our where we are after saving
        if t % 10000 == 0:
...
def main():
    ...
    inp, out = createGraph()
    #train our graph on input and output session variables
